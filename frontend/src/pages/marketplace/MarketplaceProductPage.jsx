...
import { ArrowLeft, Lock, MapPin, Package, Phone, Share2, Store, User } from 'lucide-react';
 ...
 *
 * Bouton retour (§ décidé en conversation) — un utilisateur DÉJÀ connecté
 * ne doit jamais être renvoyé vers la page MARCHÉ publique (/marche,
 * redondante avec /account qui affiche déjà le même catalogue) : le
 * bouton retour, dans les deux endroits où il apparaît sur cette page,
 * pointe donc vers /account pour un connecté, et vers /marche seulement
...
export default function MarketplaceProductPage() {
  ...
  const backTo = isAuthenticated ? '/account' : '/marche';
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-6 text-center">
        ...
        >
          <ArrowLeft size={16} />
          {isAuthenticated ? "Retour à l'accueil" : 'Retour au marché'}
        </button>
      </div>
    );
  }
      ...
      <div className="relative h-[42vh] sm:h-[48vh] bg-slate-200">
        ...
        <div className="absolute top-0 inset-x-0 flex items-center justify-between px-4 pt-[max(1rem,env(safe-area-inset-top))]">
          <button
            ...
            aria-label={isAuthenticated ? "Retour à l'accueil" : 'Retour au marché'}
          >
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mt-7 rounded-2xl bg-white border border-slate-200/80 shadow-sm p-5">
            ...
            {isAuthenticated ? (
              ...
              <button
                onClick={() =>
                  navigate('/login', { state: { redirectTo: `/marche/produits/${id}` } })
                }
                className="mt-4 w-full inline-flex items-center justify-center gap-1.5 rounded-xl bg-slate-800 text-white text-sm font-semibold py-2.5 hover:bg-slate-900 transition"
              >
