...
class _MyStorePageState extends State<MyStorePage> {
  ...
  String? _cityChoice; // valeur du menu ville (Guinée) : nom connu, ou _kOtherCity

  bool get _isGuinee => _country == 'Guinée';
  List<String> get _citiesForRegion =>
      _isGuinee && _guineeRegion != null ? (kGuineeRegions[_guineeRegion!] ?? []) : [];

  StoreTypeOption? get _selectedStoreType =>
      _storeTypes.where((t) => t.id == _storeTypeId).cast<StoreTypeOption?>().firstWhere((_) => true, orElse: () => null);
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text.rich(
          TextSpan(
            ...
            children: [
              TextSpan(text: '"$storeName" ', style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.black87)),
              const TextSpan(
                text: "a été suspendue par l'administrateur de la plateforme. Contactez l'administrateur pour en savoir plus ou pour la réactiver.",
              ),
            ],
          ),
  ...
  Widget build(BuildContext context) {
    ...
    // juste employé ailleurs.
    final alreadyOwnsStore = stores.any((s) => s.roleCode == 'OWNER');
    ...
    return ListView(
      ...
      children: [
        ...
        Text(
          alreadyOwnsStore ? 'Gérez votre boutique.' : 'Gérez vos boutiques ou créez-en une nouvelle.',
          ...
          ...stores.map((store) {
            ...
            return Card(
              ...
              child: ListTile(
                title: Row(
                  children: [
                    Flexible(child: Text(store.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600))),
                    if (store.status == 'SUSPENDED') ...[
                      const SizedBox(width: 8),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                        decoration: BoxDecoration(color: Colors.red.shade50, borderRadius: BorderRadius.circular(20)),
                        child: Text('Suspendue', style: TextStyle(color: Colors.red.shade600, fontSize: 11, fontWeight: FontWeight.w600)),
                      ),
                    ],
                  ],
                ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(labelText: 'Nom de la boutique', hintText: 'Camara Mobile Store'),
                      validator: (v) => (v == null || v.trim().isEmpty) ? 'Nom requis' : null,
                    ),
                    const SizedBox(height: 12),
                    DropdownButtonFormField<int>(
                      initialValue: _storeTypeId,
                      decoration: InputDecoration(labelText: _storeTypesLoading ? 'Chargement...' : 'Type de boutique'),
                      hint: const Text('Choisir un type...'),
                      items: [for (final t in _storeTypes) DropdownMenuItem(value: t.id, child: Text(t.label))],
                      onChanged: _storeTypesLoading ? null : (value) => setState(() => _storeTypeId = value),
                      validator: (v) => v == null ? 'Le type de boutique est requis' : null,
                    ),
                    ...
                    Padding(
                      ...
                      child: Text('LOCALISATION', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.grey.shade500, letterSpacing: 0.4)),
                    ),
                    ...
                    if (_isGuinee && _cityChoice == _kOtherCity) ...[
                      ...
                      TextFormField(
                        ...
                        decoration: const InputDecoration(labelText: 'Précisez la ville'),
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(labelText: 'Adresse (quartier)', hintText: 'Ex : Quartier Timbo, non loin du marché central'),
                    ),
                    const SizedBox(height: 12),
                    TextFormField(
                      controller: _phoneController,
                      keyboardType: TextInputType.phone,
                      decoration: const InputDecoration(labelText: 'Numéro de la boutique', hintText: '622 00 00 00'),
                    ),
                    const SizedBox(height: 20),
                    Row(
                      children: [
