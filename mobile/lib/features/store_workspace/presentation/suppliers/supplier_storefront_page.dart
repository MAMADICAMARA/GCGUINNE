...
class _SupplierStorefrontPageState extends State<SupplierStorefrontPage> {
  ...
  List<String> get _categories => (_catalog?.products.map((p) => p.categoryName).whereType<String>().toSet().toList() ?? [])..sort();
  ...
  void _addToCart(SupplierProduct product, int quantity) {
    ...
    setState(() {
      ...
      if (index >= 0) {
        ...
        _cart.add(SupplierCartItem(supplierProductId: product.id, productName: product.name, quantity: quantity, unitPrice: product.sellingPrice));
      }
    });
  }
  ...
  Future<void> _submitOrder() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _successMessage = 'Commande créée — retrouvez-la dans "Achats" pour la marquer reçue.';
        _cart.clear();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _loading
          ...
          : _loadError != null
              ? Padding(padding: const EdgeInsets.all(20), child: Text(_loadError!, style: const TextStyle(color: Colors.red)))
              : Column(
                  children: [
                    Expanded(
                      child: ListView(
                        ...
                        children: [
                          ...
                          if (_categories.isNotEmpty) ...[
                            ...
                            SizedBox(
                              ...
                              child: ListView(
                                ...
                                children: [
                                  ChoiceChip(label: const Text('Toutes catégories', style: TextStyle(fontSize: 12)), selected: _selectedCategory == null, onSelected: (_) => setState(() => _selectedCategory = null)),
                                  for (final category in _categories) ...[
                                    const SizedBox(width: 6),
                                    ChoiceChip(label: Text(category, style: const TextStyle(fontSize: 12)), selected: _selectedCategory == category, onSelected: (_) => setState(() => _selectedCategory = category)),
                                  ],
                                ],
                              ),
                            ),
                          ...
                          if (_filteredProducts.isEmpty)
                            Padding(
                              ...
                              child: Center(child: Text('Aucun produit trouvé.', style: TextStyle(color: Colors.grey.shade500))),
                            )
                          else
                            GridView.builder(
                              shrinkWrap: true,
            ...
            child: Column(
              children: [
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Expanded(child: Text(item.productName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13))),
                                  InkWell(
                                    onTap: () {
                                      setState(() => _cart.removeWhere((i) => i.supplierProductId == item.supplierProductId));
                                      setSheetState(() {});
                                    },
                              ...
                              Row(
                                children: [
                                  Expanded(
                                    child: TextField(
                                      ...
                                      onChanged: (value) {
                                        ...
                                        setState(() => item.quantity = (qty == null || qty < 1) ? 1 : qty);
                                        setSheetState(() {});
                                      },
                                    ),
                                  ),
                                  ...
                                  Expanded(
                                    child: TextField(
                                      controller: TextEditingController(text: item.unitPrice.toStringAsFixed(0)),
                                      keyboardType: TextInputType.number,
                                      decoration: const InputDecoration(labelText: 'Prix/u.', isDense: true, border: OutlineInputBorder()),
                                      onChanged: (value) {
                                        final price = num.tryParse(value);
                                        setState(() => item.unitPrice = price ?? 0);
                                        setSheetState(() {});
                                      },
                                    ),
                                  ),
                              ...
                              Align(alignment: Alignment.centerRight, child: Text(formatGNF(item.lineTotal), style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13))),
                            ],
                          ),
                        ),
                    ],
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'Référence (optionnel)', hintText: 'Ex : BC-2026-014', border: OutlineInputBorder(), isDense: true),
                ),
                const SizedBox(height: 10),
                Row(
                  children: [
                    ...
                    const Spacer(),
                    Text(formatGNF(_total), style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                  ],
                ),
                const SizedBox(height: 12),
                FilledButton(
                  onPressed: (_cart.isEmpty || _submitting || !_allowsPurchaseOrders)
                      ? null
                      : () {
                  ...
                  child: Text(!_allowsPurchaseOrders ? 'Réservé au plan PREMIUM' : (_submitting ? 'Envoi...' : 'Commander')),
                ),
              ],
            ),
          ),
...
class _CartSummaryBar extends StatelessWidget {
  const _CartSummaryBar({required this.itemCount, required this.total, required this.onTap});
...
class _SupplierProductCard extends StatefulWidget {
  const _SupplierProductCard({required this.product, required this.cartQuantity, required this.onAdd});
              ...
              if (inCart)
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: Text('${widget.cartQuantity} au panier', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w700)),
                  ),
                ),
            ],
          ),
          Padding(
            ...
            child: Column(
              ...
              children: [
                SizedBox(
                  height: 32,
                  child: Text(product.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, height: 1.15)),
                ),
                const SizedBox(height: 3),
                Text(formatGNF(product.sellingPrice), maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 14.5, fontWeight: FontWeight.w800, color: primary)),
                const SizedBox(height: 2),
                Text(product.categoryName ?? 'Sans catégorie', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 10.5, color: Colors.grey.shade500)),
                const SizedBox(height: 8),
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    _StepperButton(icon: Icons.remove, onTap: () => _changeQuantity(-1)),
                    SizedBox(width: 32, child: Text('$_quantity', textAlign: TextAlign.center, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700))),
