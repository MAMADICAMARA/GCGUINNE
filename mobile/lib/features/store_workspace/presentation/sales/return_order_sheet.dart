...
Future<bool?> showReturnOrderSheet(BuildContext context,
    {required int orderId, required List<OrderItemDetail> items}) {
  final returnable = items.where((item) => item.availableToReturn > 0).toList();
  return showModalBottomSheet<bool>(
    context: context,
    isScrollControlled: true,
    builder: (context) =>
        _ReturnOrderSheet(orderId: orderId, returnableItems: returnable),
  );
}

class _ReturnOrderSheet extends StatefulWidget {
  const _ReturnOrderSheet(
      {required this.orderId, required this.returnableItems});
...
class _ReturnOrderSheetState extends State<_ReturnOrderSheet> {
  ...
  num get _allTotal => widget.returnableItems
      .fold(0, (sum, item) => sum + item.availableToReturn * item.unitPrice);

  num get _partialTotal => widget.returnableItems.fold(0, (sum, item) {
        final qty =
            (_partialQuantities[item.id] ?? 0).clamp(0, item.availableToReturn);
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      setState(() => _error =
          "${err.message} — certains articles ont peut-être déjà été retournés avant l'erreur.");
    } finally {
      ...
      child: DraggableScrollableSheet(
        ...
        builder: (context, scrollController) => SafeArea(
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                const Text('Retourner des articles',
                    style:
                    ...
                    children: [
                        ...
                        children: [
                          ...
                          Expanded(
                            child: _ModeButton(
                                ...
                                selected: _mode == 'PARTIAL',
                                onTap: () => setState(() => _mode = 'PARTIAL')),
                          ),
                        ],
                      ),
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            for (var i = 0;
                                i < widget.returnableItems.length;
                                i++)
                              Container(
                                padding: const EdgeInsets.symmetric(
                                    horizontal: 12, vertical: 10),
                                              ...
                                              Expanded(
                                                  child: Text(
                                                      ...
                                                      style: const TextStyle(
                                                          fontSize: 13))),
                                              Text(
                                                  'Disponible : ${widget.returnableItems[i].availableToReturn}',
                                                  style: TextStyle(
                                                      fontSize: 10.5,
                                                      color: Colors
                                                          .grey.shade500)),
                      ...
                      Row(
                        children: [
                          ...
                          Text(
                              formatGNF(
                                  _mode == 'ALL' ? _allTotal : _partialTotal),
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis,
                              style: const TextStyle(
                                  fontWeight: FontWeight.w700, fontSize: 13)),
      ...
      child: Container(
        ...
        child: Text(label,
            style: TextStyle(
                ...
                fontSize: 12.5,
                color:
                    selected ? Colors.amber.shade800 : Colors.grey.shade600)),
      ),
    );
  }
}
