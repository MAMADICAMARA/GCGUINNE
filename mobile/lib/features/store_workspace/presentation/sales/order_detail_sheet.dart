...
class _OrderDetailSheetState extends State<_OrderDetailSheet> {
  ...
  Future<void> _handleDownloadInvoice() async {
    ...
    await shareInvoicePdf(context,
        orderId: widget.orderId, orderNumber: _detail!.orderNumber);
    if (mounted) setState(() => _downloadingInvoice = false);
  }

  Future<void> _openReturn() async {
    if (_detail == null) return;
    final didReturn = await showReturnOrderSheet(context,
        orderId: widget.orderId, items: _detail!.items);
    if (didReturn == true) {
      _changed = true;
      await _load();
    }
              ...
              if (detail == null)
                ...
                GridView.count(
                  ...
                  children: [
                    ...
                    _Info(
                        label: 'Date', value: formatDateTime(detail.createdAt)),
                    _Info(label: 'Vendeur', value: detail.sellerName ?? '—'),
                    _Info(
                        label: 'Client',
                        value: detail.customerName ?? 'Anonyme'),
                    _Info(
                        label: 'Paiement',
                        value: kPaymentMethodLabels[detail.paymentMethod] ??
                            detail.paymentMethod),
                    _Info(
                        label: 'Statut',
                        value:
                            kOrderStatusLabels[detail.status] ?? detail.status),
                  ],
                ),
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (detail.items[i].returnedQuantity > 0)
                                Text(
                                    '${detail.items[i].returnedQuantity} retourné(s)',
                                    style: TextStyle(
                                        fontSize: 10.5,
                ...
                _totalsLine(
                    'Réduction', '-${formatGNF(detail.discountAmount)}'),
                ...
                if (detail.amountPaid < detail.totalAmount) ...[
                  ...
                  _totalsLine('Reste à payer (cette vente)',
                      formatGNF(detail.totalAmount - detail.amountPaid),
                      bold: true, color: Colors.red.shade600),
                ],
                  ...
                  children: [
                    ...
                    if (canAct)
                      OutlinedButton(
                        ...
                        child: Text(
                            _voiding ? 'Annulation...' : 'Annuler la vente'),
                      ),
  ...
  Widget _totalsLine(String label, String value,
      ...
      child: Column(
        ...
        children: [
          Text(label.toUpperCase(),
              style: TextStyle(
                  fontSize: 9, color: Colors.grey.shade500, letterSpacing: 0.3),
              maxLines: 1,
              overflow: TextOverflow.ellipsis),
          Text(value,
              style:
